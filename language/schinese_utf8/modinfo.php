<?php
// Module 
//
define("_oscgiv_admin_funds","修改捐赠用途");


define("_oscgiv_MOD_NAME","OSC 捐助模块");
define("_oscgiv_MOD_DESC","Xoops OSC 成员捐助 模块");
define("_oscgiv_ADMENU0","OSC 捐助 管理");
//Field names
//define("_oscdir_userfile","教堂照片");

define("_oscgiv_save","保存");
define("_oscgiv_select","选择");
define("_oscgiv_create","创建");

define("_oscgiv_view","查看");

define("_oscgiv_defaultcountry_value",22);

define("_oscgiv_defaultcountry_text"," &nbsp;&nbsp;( 默认的国家是美国)");
define("_oscgiv_accessdenied","访问被拒绝");
define("_oscgiv_assignedenvelopes","已分配信封");
define("_oscgiv_activeenvelopes","近两年捐赠数");
define("_oscgiv_highestenvelopenumber","最高信封号码");
define("_oscgiv_autoassignevelopeslink","自动向清单中未分配信封的成员指配信封");
define("_oscgiv_reassignenvelopeslink","根据姓名字典序向所有存在成员分配信封");
define("_oscgiv_displaylistassignedenvelopes","显示所有指配信封列表");

define("_oscgiv_donationenvelopemgt","捐赠信封管理");
define("_oscgiv_nav_donationenvelopes","捐赠信封");

define("_oscgiv_nav_donationbatchentry","批量输入捐助");

define("_oscgiv_envelopeassignment_success","信封指派成功");

define("_oscgiv_membersaffected","受影响的成员");

define("_oscgiv_NOONEINCART","清单中没有人可以指配信封");

define("_oscgiv_ENVELOPEASSIGNSUCCESS","信封指派成功");

define("_oscgiv_envelopesassigned","被指定的信封:");

define("_oscgiv_envelopassignconfirm","进行信封指定?");

define("_oscgiv_donationbatchentry_title","批量输入捐助");

define("_oscgiv_batchentrymode","批量输入捐赠模式");
define("_oscgiv_defaultdate","默认日期");
define("_oscgiv_defaultfund","默认用途");
define("_oscgiv_none","无");
define("_oscgiv_defaulttype","默认的支付类型");
define("_oscgiv_nonte","无");
define("_oscgiv_cash","现金");
define("_oscgiv_check","检查");
define("_oscgiv_credit","信用");
define("_oscgiv_clear","清除");
define("_oscgiv_receiptnumber","收据号码");
define("_oscgiv_delete","删除");
define("_oscgiv_donationdate","捐赠日期");
define("_oscgiv_dateerror","日期错误");
define("_oscgiv_envelopenumorlastname","信封编号或姓氏");
define("_oscgiv_search","搜索");
define("_oscgiv_lookup","查找");
define("_oscgiv_submitdonations","提交捐赠");
define("_oscgiv_selecteddonor","选择的捐赠人");
define("_oscgiv_nopersonwithlastnameassignedtoenvelope","没有人被指派给信封");

define("_oscgiv_envelopnumber","信封号码");
define("_oscgiv_envelopeerror","信封错误");
define("_oscgiv_invalidenvelopenumber","非法的信封号码");

define("_oscgiv_paymenttype","支付类型");
define("_oscgiv_addressinfo","地址信息");

define("_oscgiv_fund","用途");
define("_oscgiv_amount","总计");

define("_oscgiv_applydefaults","套用默认值");

define("_oscgiv_nav_donationmanagement","捐赠管理");
define("_oscgiv_nonamefound","不匹配");
define("_oscgiv_checknumber","校验码");
define("_oscgiv_reporttitle","捐赠报告");
define("_oscgiv_submit","提交");

define("_oscgiv_admin_givingsettings","修改 Giving 设置");
define("_oscgiv_admin_permissions","修改权限");
define("_oscgiv_yearlydonationreport","年度捐赠报告");
define("_oscgiv_fax","传真");
define("_oscgiv_phone","电话");
define("_oscgiv_website","网站");
define("_oscgiv_donationreport","捐赠报告");
define("_oscgiv_donationreports_TITLE","捐赠报告");
define("_oscgiv_donationdates","捐赠日期");

define("_oscgiv_grandtotal","总和");
define("_oscgiv_total","总计");
define("_oscgiv_totalbyfundfor","总计捐助给");
define("_oscgiv_donationenvelopelist","捐赠信封列表");
define("_oscgiv_nomembers","没有成员");
define("_oscgiv_fundupdatesuccess","基金更新成功");
define("_oscgiv_fundcreatesuccess","基金创建成功");
?>
