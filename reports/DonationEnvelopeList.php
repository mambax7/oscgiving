...
function EnvelopeTable($header,$data,$startIndex,$size)
{
	...
	for($i=0; $i < $maxColumnLength; $i++)
	{
		if ($Column1Size > 0) {
			$row = $startIndex + $i;
			$sName1 = FormatFullName('', $data[$row][1], $data[$row][2], $data[$row][3], $data[$row][4], 1);

			if (strlen($data[$row][5]) > 0)
				$sAddress1 = $data[$row][5];
			else
				$sAddress1 = $data[$row][6];

			$sEnvelope1 = $data[$row][0];
			$Column1Size--;
		} else {
			$sName1 = "";
			$sAddress1 = "";
		...
		if ($Column2Size > 0) {
			...
			$sName2 = FormatFullName('', $data[$row][1], $data[$row][2], $data[$row][3], $data[$row][4], 1);

			if (strlen($data[$row][5]) > 0)
				$sAddress2 = $data[$row][5];
			else
				$sAddress2 = $data[$row][6];

			$sEnvelope2 = $data[$row][0];
			$Column2Size--;
		} else {
			$sName2 = "";
			$sAddress2 = "";
...
while ($count > 0)
{
	...
	$pdf->AddPage();
	$pdf->EnvelopeTable($header,$aData,$startIndex,$size);
