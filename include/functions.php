...
function oscgiving_getModuleAdminLink($moduleName='test') {
	global $icmsModule;
	...
	$ret = '';
	if ($moduleName) {
		$ret = "<a href='" . ICMS_URL . "/modules/$moduleName/admin/index.php'>" ._MD_TEST_ADMIN_PAGE . "</a>";
	}
	return $ret;
}
 ...
 * @todo to be move in icms core
 */
function oscgiving_getModuleName($withLink = true, $forBreadCrumb = false, $moduleName = false) {
	if (!$moduleName) {
		global $icmsModule;
		$moduleName = $icmsModule->getVar('dirname');
	}
 ...
 */
function oscgiving_getPreviousPage($default=false) {
	global $impresscms;
 ...
 */
function oscgiving_getMonthNameById($month_id) {
...
*/
function oscgiving_getLinkedUnameFromId($userid, $name = false, $users = array (), $withContact = false)
{
